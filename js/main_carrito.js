function renderProductosCarrito() {
    ...
    if (totalCarrito() > 0) {
        ...
        for (let producto of productos_carrito) {
            ...
            <td class="align-middle"><button class="btn btn-warning rounded-circle" title="Eliminar Item" onClick="eliminarItemProducto(${producto.id});"> - </button> ${producto.cantidad} <button class="btn btn-warning rounded-circle" title="Agregar Item" onClick="agregarItemProducto(${producto.id});"> + </button></td>
